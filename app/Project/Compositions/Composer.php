...
use \DemocracyApps\CNP\Project\Compositions\Inputs\ElementGenerator;
use DemocracyApps\CNP\Services\JsonProcessor;
use DemocracyApps\CNP\Utility\TableBackedObject;

class Composer extends  TableBackedObject {
    static  $tableName = 'composers';
    static protected $tableFields = array('name', 'description', 'specification',
                                            'project', 'dependson', 'contains', 'output',
    ...
    public function loadSpecification($file)
    {
        ...
        if (array_key_exists('baseSpecificationId', $cfig))
            $this->dependson = $cfig['baseSpecificationId'];
        else
            $this->dependson = null;
        return true;
    }
    ...
    private function commonProcessInput (Composition $composition, $data, $elementsSpec, $relationsSpec, $project)
    {
        ...
        if ($this->referent) {
            ...
            foreach($referents as $ref) {
                $relations = Relation::createRelationPair($ref->id,
